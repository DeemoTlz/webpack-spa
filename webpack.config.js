...
// 使用 WEBPACK_SERVE 环境变量检测当前是否是在 开发环境启动
const dev = Boolean(process.env.WEBPACK_SERVE)

module.exports = {
  /**
   * webpack 执行模式:
   * development: 开发模式，它会在配置文件中插入调试相关的选项，比如 moduleId 使用文件路径方便调试
   * production: 生产模式，webpack 会将代码做压缩等优化
   */
  ...
  module: {
    ...
    rules: [
      ...
      },
      {
         ...
         *  template.html 的文件内容会被转成一个 js 字符串，合并到 js 文件中
         */
        use: 'html-loader'
      },
      {
        ...
        test: /\.css$/,
        /**
         * 先试用 css-loader 处理，返回的结果交给 style-loader 处理
         * css-loader 将 css 内容存为 js 字符串，并且会把 background/@font-face 等引用的图片、字体等文件交给指定的 loader 打包，
         * 类似上面的 html-loader，用什么 loader 统一在 loaders 对象中定义，等会下面就会看到
         */
      ...
      },
      {
        /**
         * 匹配各种格式的图片和字体文件
         * 上面的 html-loader 会把 html 中 <img> 标签的图片解析出来，文件名匹配搭到这里的 test 的正则表达式
         * css-loader 引用的图片和字体同样会匹配到这里的 test 条件
         */
        test: /\.(png|jpg|jpeg|gif|eot|ttf|woff|woff2|svg|svgz)(\?.+)?$/,
        /**
         * 使用 url-loader，它接收一个 limit 参数，单位为字节（byte）
         * 当文件体积小于 limit 时，url-loader 会把文件转为 Date URI 的格式内联到引用的地方
         * 当文件体积大于 limit 时，url-loader 会调用 file-loader 把文件存储到输出目录，并把引用的文件路径改写成输出后的路径
         * 比如 views/foo/index.html 中，
         * <img src="smallpic.png"> 会被编译成
         * <img src="data:image/png;base64,KHASDKnJKLJLAKHiohjioKJKL...">
  ...
  /**
   * 配置 webpack 插件
   * plugin 和 loader 的区别是：loader 是在 import 时根据不同的文件名，匹配到不同的 loader 对这个文件做处理
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
       ...
       * html-loader 编译后产生的字符串会由 html-webpack-plugin 存储为 html 文件到输出目录，默认文件名为：index.html
       * 可以通过 filename 参数指定输出的文件名
       * html-webpack-plugin 也可以不指定 template 参数，他会使用默认的 html 模板
       */
...
if (dev) {
  module.exports.serve = {
    ...
    add: app => {
       ...
       * 配置 SPA 入口
       *
       * SPA 的入口是一个统一的 html 文件，
       * 比如：http://localhost:8080/foo
       * 我们要返回给它：http://localhost:8080/index.html
       */
